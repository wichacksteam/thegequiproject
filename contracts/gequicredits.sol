...
mapping (uint => GequiCreditHolder) public CreditHolders;
...
function registerCreditHolder(string memory name, uint holderId, uint creditsHeld, uint pricepercredit, uint creditvalidityperiod) public {
    totalRegistered++;
    CreditHolders[totalRegistered] = GequiCreditHolder(
        name,
        totalRegistered,
        holderId,
        creditsHeld,
...
function getCreditHoilders() public view returns (GequiCreditHolder[] memory){
    GequiCreditHolder[] memory ret = new GequiCreditHolder[](verifiercount);
